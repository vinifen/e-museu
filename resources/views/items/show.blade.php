...
<div class="container main-container mb-auto">
    ...
    @foreach($errors->all() as $error)
        <p class="error-div text-wrap fw-bold m-1 mb-4 p-1"><i class="bi bi-exclamation-circle-fill mx-1 h5"></i> {{ $error }}</p>
    @endforeach
    <div class="row">
        <div class="col-md-4 order-md-2">
            <div>
                <div class="dropdown">
                    <a type="button" class="nav-link p-3 fw-bold explore-button dropdown-toggle" data-bs-toggle="collapse" data-bs-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
                        <i class="bi bi-plus-circle h4 me-2"></i> CONTRIBUIR
                    </a>
                    <div class="collapse" id="collapseExample">
                        <ul class="list-group p-2">
                            <a class="nav-link py-3 fw-bold" href="" data-bs-toggle="modal" data-bs-target="#addContributionModal">
                                <i class="bi bi-patch-plus-fill h4 me-2"></i>Agregar Conteúdo
                            </a>
                            <a class="nav-link py-3 fw-bold" href="" data-bs-toggle="modal" data-bs-target="#addComponentModal">
                                <i class="bi bi-cpu h4 me-2"></i>Associar Componente
                            </a>
                            <a class="nav-link py-3 fw-bold" href="" data-bs-toggle="modal" data-bs-target="#addTagModal">
                                <i class="bi bi-tag-fill h4 me-2"></i>Associar Etiqueta
                            </a>
                            <a class="nav-link py-3 fw-bold" href="" data-bs-toggle="modal" data-bs-target="#addExtraModal">
            ...
            <div class="card mt-2">
                ...
                <div style="overflow:hidden;">
                    <img src="{{ url("storage/{$item->image}") }}" class="card-img-top p-1 clickable-image" style="aspect-ratio: 1 / 1; width: 100%; max-height: 100%; object-fit: cover" alt="Imagem do item">
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-5">
                        ...
                        <div class="col-md-7">
                            <a href={{ route('items.index', ['section' => $item->section->id]) }} ><p class="show-item-link">{{ $item->section->name }}</p></a>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-5">
                    ...
                    @foreach($item->tagItems as $tagItem)
                        @if ($tagItem->validation == 1)
                            <div class="row">
                                ...
                                <div class="col-md-7">
                                    <a href={{ route('items.index', ['tag[]' => $tagItem->tag->id]) }} ><p class="show-item-link">{{ $tagItem->tag->name }}</p></a>
                                </div>
                            </div>
                        @endif
                    @endforeach
                </div>
                <div><h5>Detalhes Técnicos</h5></div>
                <div class="card-body">
                    <p>{{ $item->detail }}</p>
                </div>
                <div><h5>Componentes</h5></div>
                        ...
                        <div class="col-6 p-2">
                            <a href={{ route('items.show', $ItemComponent->component->id) }}>
                                <div class="card-anim component-button p-1">
                                    <div class="">
                                        <img src="{{ url("storage/{$ItemComponent->component->image}") }}" class="component-img p-1" alt="Imagem do componente">
                                    </div>
                                    <div class="p-1">
                                        <p class="mb-1 fw-bold"> {{ Str::limit($ItemComponent->component->name, 30) }}</p>
                                        <p class="mb-0"> {{ Str::limit($ItemComponent->component->section->name) }}</p>
                                    </div>
                                </div>
                            </a>
                        </div>
        ...
        <div class="col-md-8 order-md-1">
                                    ...
                                    @if ($timelineItem->validation == 1)
                                        <div class="my-4">
                                            <div class="d-flex align-items-start">
                                                ...
                                                <h6 class="fw-bold timeline-item-date">{{ date('d/m/Y', strtotime($timelineItem->date)) }}</h6>
                                            </div>
                                            <div class="d-md-flex">
                                                <div>
                                                    <img src="{{ url("storage/{$timelineItem->image}") }}" class="card-img-top p-1 clickable-image" style="width: 12rem; height: 12rem; object-fit: cover" alt="Imagem do item da série">
                                                </div>
                                                <div class="ms-2">
                                                    <p class="fw-bold">{{ $timelineItem->name }}</p>
                                                    <p>{{ $timelineItem->description }}</p>
                                                    <a href={{ route('items.show', $timelineItem->id) }}><h6 class="me-4 d-flex justify-content-end card-more-details">MAIS DETALHES ></h6></a>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                ...
                @foreach($item->extras as $extra)
                    @if ($extra->validation == '1')
                        <div class="m-4">
                            <p>{{ $extra->info }}</p>
                            <div class="row">
                                <p class="fw-bold col-2">Adicionado por </p><p class="col-10">{{ $extra->proprietary->full_name }}</p>
                            </div>
            ...
            @else
                <div class="m-4">
                    <strong>Nenhuma Curiosidade extra validada está disponível para este item. Nos ajude adicionando uma curiosidade!</strong>
                </div>
            @endif
        </div>
    </div>
