...
@section('content')
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="component_section_id" class="form-label">Seção do Componente</label>
                                <select class="form-select @error('component_section_id') is-invalid @enderror" id="component_section_id" name="component_section_id">
                                    @foreach($sections as $section)
                                    <option value="{{ $section->id }}" {{ old('component_section_id') == $section->id ? 'selected' : '' }}>{{ $section->name }}</option>
                                    @endforeach
                                </select>
                                @error('component_section_id') <div class="invalid-feedback"> {{ $message }} </div> @enderror
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="component_id" class="form-label">Componente</label>
                                <select class="form-select @error('component_id') is-invalid @enderror" id="component_id" name="component_id">
                                    <option value="">-</option>
                                </select>
                                @error('component_id') <div class="invalid-feedback"> {{ $message }} </div> @enderror
                            </div>
                        </div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <button type="submit" class="btn btn-success"><i class="bi bi-plus-circle"></i> Adicionar Componente</button>
                    </div>
                </div>
            </div>
        </form>
    ...
    <script src="{{ asset('script/getComponentBySection.js') }}"></script>
@endsection
