...
@section('title', 'Editar contribuição ' . $contribution->id)
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea type="text" class="form-control @error('content') is-invalid @enderror" id="content" name="content"
                            rows="5">{{ $contribution->content }}</textarea>
                        @error('content')
                                ...
                                <select class="form-select @error('section_id') is-invalid @enderror" id="section_id"
                                    ...
                                    @foreach ($sections as $section)
                                        <option value="{{ $section->id }}"
                                            {{ $contribution->item->section->id == $section->id ? 'selected' : '' }}>
                                            {{ $section->name }}</option>
                                    @endforeach
                                </select>
                                @error('section_id')
                        ...
                        <select class="form-select @error('proprietary_id') is-invalid @enderror" id="proprietary_id"
                            ...
                            @foreach ($proprietaries as $proprietary)
                                <option value="{{ $proprietary->id }}"
                                    {{ $contribution->proprietary->id == $proprietary->id ? 'selected' : '' }}>
